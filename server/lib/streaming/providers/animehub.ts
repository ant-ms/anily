...
const log = logger.child({ provider: "animehub" });
...
export class AnimeHubProvider implements BaseProvider {
  ...
  async search(query: string): Promise<ProviderSearchResult[]> {
    ...
    try {
      ...
      const itemRegex = /<div class="item">[\s\S]*?<a href="([^"]+)"[^>]*class="poster"[\s\S]*?(?:<span class="(dub|sub)">([^<]+)<\/span>)?[\s\S]*?<a [^>]*class="name">([^<]+)<\/a>/g;
      ...
      while ((match = itemRegex.exec(html)) !== null) {
        ...
        const isDub = identifier.endsWith("-dub") || langClass === "dub" || name.toLowerCase().endsWith("(dub)");

        if (identifier.endsWith("-dub")) {
          identifier = identifier.slice(0, -4);
        }

        name = name.replace(/\s*\((?:Dub|Sub)\)\s*$/i, "").replace(/\s+Dub\s*$/i, "").trim();
      ...
      log.error({ err, query }, "Error searching AnimeHub");
      return [];
    }
  }

  async getEpisodes(identifier: string, lang: StreamLanguage): Promise<{ episodes: number[]; servers: Array<{ id: string; name: string }> }> {
    ...
    try {
      ...
      while ((epMatch = epRegex.exec(html)) !== null) {
        ...
        if (!isNaN(epNum)) {
          epEpisodesAdd(episodesSet, epNum);
        }
      }
      ...
      log.error({ err, identifier, lang }, "Failed to get episodes from AnimeHub");
      return { episodes: [], servers: [] };
    }
  }

  async getStream(identifier: string, episode: number, lang: StreamLanguage, server = "0"): Promise<StreamSource | null> {
    const slug = lang === "dub" ? `${identifier}-dub` : identifier;
    const animeUrl = `${BASE_URL}/anime/${slug}`;
    ...
    for (const s of candidateServers) {
      try {
        ...
        const embedRes = await fetch(target, {
          headers: {
            ...DEFAULT_HEADERS,
            Referer: animeUrl,
          },
        ...
        const srcMatch = hsHtml.match(/<div[^>]*id=["']sources["'][^>]*>(.*?)<\/div>/s);
        ...
        return {
          ...
          serverName: s === "0" ? "F5 - HQ" : s === "10" ? "No Ads 4" : `Server ${s}`,
        };
      } catch (err) {
        log.warn({ err, slug, episode, server: s }, "Server attempt failed, trying next");
      }
    }
...
function epEpisodesAdd(set: Set<number>, num: number) {
  set.add(num);
}
