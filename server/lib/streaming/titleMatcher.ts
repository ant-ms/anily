...
const log = logger.child({ module: "titleMatcher" });
...
function extractSeasonNumber(title: string): number | null {
  ...
  if (t.includes("2nd season") || t.includes("season 2") || t.includes("season ii")) return 2;
  if (t.includes("3rd season") || t.includes("season 3") || t.includes("season iii")) return 3;
  if (t.includes("4th season") || t.includes("season 4") || t.includes("season iv")) return 4;
  return null;
}
...
Reply with ONLY the index number (e.g. "0" or "1") or "NONE".`;
    ...
    if (!isNaN(idx) && idx >= 0 && idx < candidates.length) {
      log.info({ targetTitle, matched: candidates[idx].name }, "OpenRouter AI SDK matched anime title");
      return candidates[idx];
    }
  } catch (err: any) {
    log.warn({ err: err.message, targetTitle }, "OpenRouter AI SDK title matcher failed, using heuristic fallback");
  }
...
export async function matchBestSearchResult(
  ...
  for (const target of cleanTargets) {
    const exact = searchResults.find((r) => normalizeTitle(r.name) === target.norm);
    if (exact) return exact;
  }

  // 2. If OpenRouter API Key is available, use LLM for ambiguous title matching
  if (process.env.OPENROUTER_API_KEY) {
    const llmMatch = await callOpenRouterFallback(targetTitles[0], searchResults);
  ...
  for (const candidate of searchResults) {
    ...
    // Matching season gets a huge boost
    if (candSeason === targetSeason) {
      score += 50;
    } else {
      score -= 50; // Penalty for wrong season (e.g. season 2 when season 1 wanted)
    }

    // Penalize movies/OVAs if target doesn't ask for them
    const isSpecial = /\b(movie|ova|special|recap)\b/i.test(candidate.name);
    const targetIsSpecial = /\b(movie|ova|special|recap)\b/i.test(primary.original);
